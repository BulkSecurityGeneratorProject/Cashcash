...
export default function TreeDnDDrag($timeout, TreeDnDHelper) {
    "ngInject";
    ...
    function _fnDragStart(e, $params) {
        ...
        var _scope = dragScope;
        var _element = _scope.$element;
        var _clone;
        var _needCollapse = !!_$scope.enabledCollapse;
        var _copied = false;
        var _tbody;
        var _frag;
    ...
    function _fnDragMove(e, $params) {
        ...
        if ($params.dragElm) {
            ...
            var targetY = eventObj.pageY - (window.pageYOffset || $params.$window.document.documentElement.scrollTop);
            var targetElm;
            var targetScope;
            var targetBefore;
            var targetOffset;
            ...
            var _target;
            var _parent;
            var _info = $params.dragInfo;
            var _move = _info.move;
            var _drag = _info.node;
            var _drop = _info.drop;
            var treeScope = _info.target;
            var fnSwapTree;
            var isHolder = _fnPlaceHolder(e, $params);
            ...
            if (treeScope.$callbacks.accept(_info, _move, isChanged)) {
                ...
                if (targetScope.isTable) {
                    TreeDnDHelper.replaceIndent(
                        treeScope,
                        $params.placeElm,
                        (_move.parent === undefined || _move.parent === null ) ? 1 : _move.parent.__level__ + 1
                    );
