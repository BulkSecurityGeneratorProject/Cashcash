...
export default function fnInitTreeDnD($timeout, $http, $compile, $parse, $window, $document, $templateCache,
    ...
    function fnController($scope, $element, $attrs) {
        ...
        var _fnSetHash = function (node) {
            var _hashKey = _fnGetHash(node);
            if (node.__hashKey__ === undefined || node.__hashKey__ === null || node.__hashKey__ !== _hashKey) {
        ...
        var _watches = [
            ...
            [
                ...
                [
                    ...
                    function (expandOn) {
                        if (expandOn === undefined || expandOn === null) {
                ...
                [
                    ...
                    function (colDefs) {
                        if (colDefs === undefined || colDefs === null || !angular.isArray(colDefs)) {
        ...
        if ($attrs.treeData) {
            $scope.$watch(
                $attrs.treeData, function (val) {
                    ...
                    tmpTreeData = val;
                    if (timeReloadData === undefined || timeReloadData === null) {
        ...
        function check_exist_attr(attrs, existAttr, isAnd) {
            if (existAttr === undefined || existAttr === null) {
                return false;
            }
        ...
        function getExpandOn() {
            if ($scope.treeData && $scope.treeData.length) {
                ...
                // Auto get first
                if ($scope.expandingProperty === undefined || $scope.expandingProperty === null) {
        ...
        function do_f(root, node, parent, parent_real, level, visible, index) {
            ...
            if ((node.__expanded__ === undefined || node.__expanded__ === null) && _len > 0) {
            ...
            if (node.__uid__ === undefined || node.__uid__ === null) {
                node.__uid__ = '' + Math.random();
            }
