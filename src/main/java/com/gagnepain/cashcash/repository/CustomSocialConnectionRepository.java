...
import com.gagnepain.cashcash.domain.SocialUserConnection;
...
public class CustomSocialConnectionRepository implements ConnectionRepository {
	...
	public CustomSocialConnectionRepository(final String userId, final SocialUserConnectionRepository socialUserConnectionRepository,
	...
	public MultiValueMap<String, Connection<?>> findAllConnections() {
		final List<SocialUserConnection> socialUserConnections = socialUserConnectionRepository.findAllByUserIdOrderByProviderIdAscRankAsc(
				userId);
		...
		for (final Connection<?> connection : connections) {
			final String providerId = connection.getKey()
					.getProviderId();
			if (connectionsByProviderId.get(providerId)
					.size() == 0) {
	...
	public List<Connection<?>> findConnections(final String providerId) {
		final List<SocialUserConnection> socialUserConnections = socialUserConnectionRepository.findAllByUserIdAndProviderIdOrderByRankAsc(
				userId, providerId);
	...
	@Override
	public MultiValueMap<String, Connection<?>> findConnectionsToUsers(final MultiValueMap<String, String> providerUserIdsByProviderId) {
	...
	public Connection<?> getConnection(final ConnectionKey connectionKey) {
		final SocialUserConnection socialUserConnection = socialUserConnectionRepository.findOneByUserIdAndProviderIdAndProviderUserId(
	...
	public void addConnection(final Connection<?> connection) {
		final Long rank = getNewMaxRank(connection.getKey()
	...
	public void updateConnection(final Connection<?> connection) {
		final SocialUserConnection socialUserConnection = socialUserConnectionRepository.findOneByUserIdAndProviderIdAndProviderUserId(
				userId, connection.getKey()
						.getProviderId(), connection.getKey()
						.getProviderUserId());
		if (socialUserConnection != null) {
			final SocialUserConnection socialUserConnectionToUdpate = connectionToUserSocialConnection(connection,
	...
	public void removeConnection(final ConnectionKey connectionKey) {
		socialUserConnectionRepository.deleteByUserIdAndProviderIdAndProviderUserId(userId, connectionKey.getProviderId(),
	...
	private SocialUserConnection connectionToUserSocialConnection(final Connection<?> connection, final Long rank) {
		...
		return new SocialUserConnection(userId, connection.getKey()
				.getProviderId(), connection.getKey()
				.getProviderUserId(), rank, connection.getDisplayName(), connection.getProfileUrl(), connection.getImageUrl(),
				connectionData.getAccessToken(), connectionData.getSecret(), connectionData.getRefreshToken(),
				connectionData.getExpireTime());
	}

	private List<Connection<?>> providerUserIdsToConnections(final String providerId, final List<String> providerUserIds) {
		final List<SocialUserConnection> socialUserConnections = socialUserConnectionRepository.findAllByUserIdAndProviderIdAndProviderUserIdIn(
				userId, providerId, providerUserIds);
		return socialUserConnectionsToConnections(socialUserConnections);
	}

	private List<Connection<?>> socialUserConnectionsToConnections(final List<SocialUserConnection> socialUserConnections) {
		return socialUserConnections.stream()
				.map(this::socialUserConnectionToConnection)
	...
	private Connection<?> socialUserConnectionToConnection(final SocialUserConnection socialUserConnection) {
		final ConnectionData connectionData = new ConnectionData(socialUserConnection.getProviderId(),
				socialUserConnection.getProviderUserId(), socialUserConnection.getDisplayName(), socialUserConnection.getProfileURL(),
				socialUserConnection.getImageURL(), socialUserConnection.getAccessToken(), socialUserConnection.getSecret(),
