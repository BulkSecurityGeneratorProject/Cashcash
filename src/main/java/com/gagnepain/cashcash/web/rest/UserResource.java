...
import com.gagnepain.cashcash.service.MailService;
...
public class UserResource {
	...
	public ResponseEntity<?> createUser(@RequestBody final ManagedUserDTO managedUserDTO, final HttpServletRequest request)
			throws URISyntaxException {
		...
		if (userRepository.findOneByLogin(managedUserDTO.getLogin()
			...
			final User newUser = userService.createUser(managedUserDTO);
	...
	@Transactional
	@Secured(AuthoritiesConstants.ADMIN)
	public ResponseEntity<ManagedUserDTO> updateUser(@RequestBody final ManagedUserDTO managedUserDTO) {
		...
		return userRepository.findOneById(managedUserDTO.getId())
				.map(user -> {
					...
					return ResponseEntity.ok()
							.headers(HeaderUtil.createAlert("A user is updated with identifier " + managedUserDTO.getLogin(),
	...
	public ResponseEntity<List<ManagedUserDTO>> getAllUsers(final Pageable pageable) throws URISyntaxException {
		...
		final List<ManagedUserDTO> managedUserDTOs = page.getContent()
				.stream()
